...
const validateStep2 = (tabs) => {
  ...
  tabs.forEach(tab => {
    ...
    if (tab.type === "新規_単店" || tab.type === "新規_店舗追加") {
      ...
      if (d.withFrontWan && !d.wanMailNotUse && !d.wanMailPrefix?.trim()) e.wanMailPrefix = "入力してください";
    }
    if (tab.type === "新規_フロントWAN本部") {
    ...
    <Card>
      ...
      {step1==="新規申込" && step2==="本部" && step3==="フロントWAN本部" && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            <SelectBtn label="既存店舗へのフロントWAN本部追加" onClick={() => onSelect("新規_フロントWAN本部")} />
            <SelectBtn label="新規申込店舗へのフロントWAN本部追加" onClick={() => setStep3("新規申込店舗フロントWAN")} />
          </div>
        </div>
      )}

      {/* 新規申込店舗へのフロントWAN本部追加：案内 */}
      {step1==="新規申込" && step2==="本部" && step3==="新規申込店舗フロントWAN" && (
        tenpoTabs.length === 0
          ? <div style={{ padding:"16px", background:"#fef9c3", border:"1.5px solid #fde047", borderRadius:10, fontSize:13.5, lineHeight:1.8 }}>
              <div style={{ fontWeight:700, marginBottom:6 }}>⚠️ 先に店舗の申込書を作成してください</div>
              <div style={{ color:C.text }}>
                単店または店舗追加の申込書を作成してから、<br />
                各店舗フォームのサービス選択欄にある<br />
                <span style={{ fontWeight:700 }}>「🏢 この店舗をフロントWAN本部（本店）にする」</span><br />
                にチェックを入れてください。
              </div>
            </div>
          : <div style={{ padding:"16px", background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:10, fontSize:13.5, lineHeight:1.8 }}>
              <div style={{ fontWeight:700, marginBottom:6, color:C.tealDark }}>✅ 店舗の申込書にチェック項目があります</div>
              <div style={{ color:C.text, marginBottom:12 }}>
                申込書のサービス選択欄にある<br />
                <span style={{ fontWeight:700 }}>「🏢 この店舗をフロントWAN本部（本店）にする」</span><br />
                にチェックを入れてください。
              </div>
              <button onClick={() => onJump(tenpoTabs[0].id)}
                style={{ padding:"9px 22px", borderRadius:10, border:"none", background:C.teal, color:"#fff", fontSize:13.5, fontWeight:700, cursor:"pointer", fontFamily:FONT }}>
                該当の申込書へ移動する →
              </button>
            </div>
      )}
  ...
  return (<>
    ...
    <Card>
      ...
      <Field label="本部名フリガナ" required hint={!te.honbuNameKana ? "手動入力（カタカナ）" : undefined} error={te.honbuNameKana}>
        <UIInput value={d.honbuNameKana||""} hasError={!!te.honbuNameKana}
    ...
    <Card>
      ...
      <div style={{ background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:12, padding:"14px 18px", marginTop:8 }}>
        <div style={{ fontSize:12.5, color:C.tealDark, fontWeight:700, marginBottom:8 }}>💰 事務所WAN本部　月額料金合計（税別）</div>
...
const FrontWanTabContent = ({ tab, form, updTab, clrTab, errors }) => {
  ...
  return (<>
    ...
    <Card>
      ...
      {/* 月額合計 */}
      <div style={{ background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:12, padding:"14px 18px", marginTop:8 }}>
        <div style={{ fontSize:12.5, color:C.tealDark, fontWeight:700, marginBottom:8 }}>💰 フロントWAN本部　月額料金合計（税別）</div>
        <div style={{ display:"flex", flexDirection:"column", gap:4, fontSize:13 }}>
          <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>WAN本部</span><span>{fmtPrice(WAN_HONBU_BASE)}</span></div>
          {d.wanLanEnabled && <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>LAN（{d.wanLanCount||1}台）</span><span>{fmtPrice(LAN_PRICE*(d.wanLanCount||1))}</span></div>}
          {d.wanConciergeManager && d.wanConciergeManager!=="0台" && (() => {
...
const TenpoTabContent = ({ tab, form, updTab, clrTab, errors }) => {
  ...
  return (<>
    ...
    <Card>
      ...
      {/* フロントWAN本部（本店）同時申込 */}
      <div style={{ marginTop:16, paddingTop:16, borderTop:`1.5px dashed ${C.teal}` }}>
        <label style={{ display:"flex", alignItems:"center", gap:10, cursor:"pointer", padding:"10px 14px", borderRadius:10, border:`1.5px solid ${d.withFrontWan?C.teal:C.border}`, background:d.withFrontWan?C.tealLight:C.white, userSelect:"none" }}>
          <input type="checkbox" checked={!!d.withFrontWan} style={{ width:16, height:16, cursor:"pointer" }}
            onChange={e => { upd("withFrontWan", e.target.checked); if (!e.target.checked) updM({ wanMailNotUse:false, wanMailPrefix:"", wanLanEnabled:false, wanLanCount:1, wanConciergeManager:"0台" }); }} />
          <span style={{ fontSize:13.5, fontWeight:d.withFrontWan?700:400, color:d.withFrontWan?C.tealDark:C.text }}>🏢 この店舗をフロントWAN本部（本店）にする</span>
        </label>

        {d.withFrontWan && (
          <div style={{ marginTop:12, paddingLeft:16, borderLeft:`3px solid ${C.teal}` }}>
            {/* 本部用メールアドレス */}
            <div style={{ fontSize:12.5, color:C.tealDark, fontWeight:700, marginBottom:10 }}>フロントWAN本部　LinQ2メールアドレス</div>
            <Field label="本部用メールアドレス" required={!d.wanMailNotUse} error={te.wanMailPrefix}>
              <div style={{ display:"flex", alignItems:"center", gap:0 }}>
                <UIInput value={d.wanMailPrefix||""} hasError={!!te.wanMailPrefix}
                  noZenkaku inputMode="email" disabled={!!d.wanMailNotUse}
                  style={{ borderRadius:"8px 0 0 8px", flex:1, minWidth:0 }}
                  onChange={e => { const v=e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,""); upd("wanMailPrefix",v); clr("wanMailPrefix"); }}
                  placeholder="例：salon-honbu" />
                <div style={{ padding:"9px 14px", background:"#f0f4f8", border:`1.5px solid ${te.wanMailPrefix?C.danger:C.border}`, borderLeft:"none", borderRadius:"0 8px 8px 0", fontSize:14, color:C.muted, whiteSpace:"nowrap", flexShrink:0 }}>
                  @tbqm.jp
                </div>
              </div>
              {!te.wanMailPrefix && d.wanMailPrefix && !d.wanMailNotUse && (
                <div style={{ marginTop:6, fontSize:12.5, color:C.teal }}>✅ 完成形：{d.wanMailPrefix}@tbqm.jp</div>
              )}
            </Field>
            <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontSize:13, fontWeight:500, color:C.text, userSelect:"none", marginBottom:14 }}>
              <input type="checkbox" checked={!!d.wanMailNotUse} style={{ width:15, height:15, cursor:"pointer" }}
                onChange={e => { updM({ wanMailNotUse:e.target.checked, wanMailPrefix:e.target.checked?"":d.wanMailPrefix }); clr("wanMailPrefix"); }} />
              LinQ2メールアドレスは使用しない
            </label>

            {/* フロントWAN本部サービス選択 */}
            <div style={{ fontSize:12.5, color:C.tealDark, fontWeight:700, marginBottom:10 }}>フロントWAN本部　サービス選択</div>
            <div style={{ padding:"10px 14px", background:"#f0f4f8", border:`1px solid ${C.border}`, borderRadius:8, fontSize:13.5, color:C.navy, fontWeight:600, marginBottom:10 }}>
              WAN本部　{fmtPrice(WAN_HONBU_BASE)}/月（固定）
            </div>
            <Field label="オプション">
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            ...
            {/* 月額合計（店舗＋WAN本部合算） */}
            {d.package && (() => {
              const wanTotal = WAN_HONBU_BASE + (d.wanLanEnabled ? LAN_PRICE*(d.wanLanCount||1) : 0) + (WAN_CONCIERGE_MANAGER_OPTIONS.indexOf(d.wanConciergeManager)>0 ? WAN_CONCIERGE_MANAGER_OPTIONS.indexOf(d.wanConciergeManager)*WAN_CONCIERGE_MANAGER_PRICE : 0);
              return (
                <div style={{ background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:12, padding:"14px 18px", marginTop:8 }}>
                  <div style={{ fontSize:12.5, color:C.tealDark, fontWeight:700, marginBottom:8 }}>💰 フロントWAN本部（本店）月額料金合計（税別）</div>
                  <div style={{ display:"flex", flexDirection:"column", gap:4, fontSize:13 }}>
                    <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>店舗　{d.package}</span><span>{fmtPrice(monthly)}</span></div>
                    <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>フロントWAN本部</span><span>{fmtPrice(wanTotal)}</span></div>
                    <div style={{ borderTop:`1px solid ${C.teal}`, marginTop:6, paddingTop:6, display:"flex", justifyContent:"space-between", fontWeight:800, fontSize:15 }}>
                      <span>合計</span><span style={{ color:C.tealDark }}>{fmtPrice(monthly+wanTotal)}</span>
                    </div>
                  </div>
                </div>
              );
