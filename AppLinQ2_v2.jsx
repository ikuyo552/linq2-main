...
const validateStep2 = (tabs) => {
  ...
  tabs.forEach(tab => {
    ...
    if (tab.type === "端末入替") {
      ...
      if (!d.denwa)                 e.denwa         = "入力してください";
      // 機器引取
      if ((d.kikiHikitori||"あり") === "あり" && !d.hikitoriKiki?.length) e.hikitoriKiki = "1つ以上選択してください";
      if ((d.kikiHikitori||"あり") === "あり" && (d.hikitoriKiki||[]).includes("その他") && !d.hikitoriSonota?.trim()) e.hikitoriSonota = "入力してください";
      // 日程調整
      if (!d.nicchoseiSaki)             e.nicchoseiSaki         = "選択してください";
      if (!d.nicchoseiTantosha?.trim()) e.nicchoseiTantosha     = "入力してください";
      if (!d.nicchoseiTantoshaKana?.trim()) e.nicchoseiTantoshaKana = "入力してください";
  ...
  return (<>
    ...
    <Card>
      ...
      <FieldRow>
        ...
        <Field label="機器引取" required half>
          <UISelect value={d.kikiHikitori||"あり"} onChange={e => {
