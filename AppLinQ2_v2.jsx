  ...
  return (<>
    ...
    <Card>
            ...
            return (
              <label key={n.key} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, border:`1.5px solid ${checked ? C.teal : te.nohinData ? C.danger : C.border}`, background:checked ? C.tealLight : te.nohinData ? "#fff5f5" : C.white, cursor:"pointer", transition:"all .15s" }}>
                <input type="checkbox" checked={checked} style={{ width:16, height:16, cursor:"pointer" }} onChange={e => {
                  ...
                  // 他社移行のチェックを外したら関連フィールドをリセット
                  if (n.key==="tasha" && !e.target.checked) updM({ tashaDataType:"", tashaEntryBook:"", netKaiTsubiStatus:"", netKaiTsubiKakutei:"確定", netKaiTsubiMode:"date", netKaiTsubiDate:"", netKaiTsubiText:"", openStatus:"", openMode:"date", openDate:"", openText:"" });
                }} />
      ...
      {hasTasha && (
        <div style={{ marginTop:4, paddingLeft:16, borderLeft:`3px solid ${C.teal}`, marginBottom:14 }}>
          <div style={{ fontSize:12.5, color:C.tealDark, fontWeight:700, marginBottom:10 }}>他社移行　詳細</div>
          <Field label="他社データの種類" required error={te.tashaDataType}>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {["他社データ（売上データ含む）","他社データ（顧客データのみ）"].map(v => (
                <label key={v} style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", padding:"9px 16px", borderRadius:10, border:`1.5px solid ${d.tashaDataType===v ? C.teal : te.tashaDataType ? C.danger : C.border}`, background:d.tashaDataType===v ? C.tealLight : te.tashaDataType ? "#fff5f5" : C.white, fontSize:13.5, fontWeight:d.tashaDataType===v?700:400 }}>
                  <input type="radio" name={`tashaData-${tabId}`} value={v} checked={d.tashaDataType===v}
                    onChange={() => { upd("tashaDataType",v); clr("tashaDataType"); }} />{v}
                </label>
              ))}
            </div>
            <div style={{ background:"#fef9c3", border:"1px solid #fde047", borderRadius:6, padding:"8px 12px", fontSize:12.5, color:"#854d0e", marginTop:8 }}>
              ⚠️ 他社データ移行費用は別途発生します。
            </div>
          </Field>
          <Field label="エントリーブック提出" required error={te.tashaEntryBook}>
            <div style={{ display:"flex", gap:12 }}>
              {["あり","なし"].map(v => (
                <label key={v} style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", padding:"9px 20px", borderRadius:10, border:`1.5px solid ${d.tashaEntryBook===v ? C.teal : te.tashaEntryBook ? C.danger : C.border}`, background:d.tashaEntryBook===v ? C.tealLight : te.tashaEntryBook ? "#fff5f5" : C.white, fontSize:13.5, fontWeight:d.tashaEntryBook===v?700:400 }}>
                  <input type="radio" name={`tashaEntry-${tabId}`} value={v} checked={d.tashaEntryBook===v}
                    onChange={() => { upd("tashaEntryBook",v); clr("tashaEntryBook"); }} />{v}
                </label>
              ))}
            </div>
          </Field>
    ...
    <Card>
      ...
      <Field label="申込パッケージ" required error={te.package}>
        ...
        {d.package && <div style={{ fontSize:12, color:C.muted, marginTop:6 }}>e-Reserve込み{d.package==="ベーシックパック+"?"・LinQ Concierge 2台込み":""}</div>}
      ...
      <Card>
        ...
        {d.shiharaiHoho==="既存口座で支払う" && (
          <div style={{ marginTop:4, paddingLeft:16, borderLeft:`3px solid ${C.teal}` }}>
            <Field label="既存の店舗名" hint="引き落とし口座を使用している店舗名を入力してください">
              <UIInput value={d.kizonTenpoName||""} onChange={e => upd("kizonTenpoName",e.target.value)} placeholder="例：ヘアサロン〇〇 新宿店" style={{ maxWidth:320 }} />
            </Field>
          </div>
        )}
      </Card>
    ...
    <Card>
      ...
      {/* 他社移行チェック時のみ：ネット開通日・オープン日 */}
      {hasTasha && (<>
        <Field label="ネット開通日" error={te.netKaiTsubiStatus}>
        ...
        <Field label="オープン日" error={te.openStatus}>
          <div style={{ display:"flex", gap:10, marginBottom:10 }}>
            {["確定","未定"].map(s => (
              <label key={s} style={{ display:"flex", alignItems:"center", gap:6, cursor:"pointer", padding:"6px 16px", borderRadius:10, border:`1.5px solid ${d.openStatus===s?C.teal:te.openStatus?C.danger:C.border}`, background:d.openStatus===s?C.tealLight:te.openStatus?"#fff5f5":C.white, fontSize:13, fontWeight:d.openStatus===s?700:400 }}>
                <input type="radio" name={`openStatus-${tabId}`} value={s} checked={d.openStatus===s} onChange={() => { upd("openStatus",s); clr("openStatus"); }} />{s}
              </label>
            ))}
          </div>
          {d.openStatus==="確定" && (
          ...
          {d.openStatus==="未定" && <div style={{ fontSize:13, color:C.muted, padding:"6px 0" }}>📝 申込書に「未定」と記載されます</div>}
        </Field>
      </>)}
