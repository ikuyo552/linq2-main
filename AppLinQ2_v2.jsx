    ...
    <Card>
      ...
      {step1==="新規申込" && step2==="本部" && step3==="フロントWAN本部" && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            <SelectBtn label="既存店舗" onClick={() => onSelect("新規_フロントWAN本部")} />
            <SelectBtn label="新規申込店舗" onClick={() => setStep3("新規申込店舗フロントWAN")} />
          </div>
        </div>
      )}
...
const TenpoTabContent = ({ tab, form, updTab, clrTab, errors }) => {
  ...
  return (<>
    {/* フロントWAN本部（本店）同時申込 */}
    <Card style={{ border:`1.5px dashed ${d.withFrontWan?C.teal:C.border}`, background:d.withFrontWan?C.tealLight:C.white }}>
      <label style={{ display:"flex", alignItems:"center", gap:10, cursor:"pointer", userSelect:"none" }}>
        <input type="checkbox" checked={!!d.withFrontWan} style={{ width:17, height:17, cursor:"pointer" }}
        ...
        <span style={{ fontSize:14, fontWeight:700, color:d.withFrontWan?C.tealDark:C.text }}>🏢 この店舗をフロントWAN本部（本店）にする</span>
      </label>

      {d.withFrontWan && (
        <div style={{ marginTop:16, paddingTop:16, borderTop:`1.5px solid ${C.teal}` }}>
          ...
          <Field label="本部用メールアドレス" required={!d.wanMailNotUse} error={te.wanMailPrefix}>
            <div style={{ display:"flex", alignItems:"center", gap:0 }}>
              <UIInput value={d.wanMailPrefix||""} hasError={!!te.wanMailPrefix}
                ...
                placeholder="例：salon-honbu" />
              <div style={{ padding:"9px 14px", background:"#f0f4f8", border:`1.5px solid ${te.wanMailPrefix?C.danger:C.border}`, borderLeft:"none", borderRadius:"0 8px 8px 0", fontSize:14, color:C.muted, whiteSpace:"nowrap", flexShrink:0 }}>@tbqm.jp</div>
            </div>
