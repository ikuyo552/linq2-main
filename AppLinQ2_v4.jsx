...
const getOpenDateLabel = d => {
  ...
  if (d.openStatus === "未定") return "未定";
  return d.openMode === "text" ? (d.openText || "") : formatDate(d.openDate);
};

const calcMonthly = d => {
  let total = PACKAGE_BASE[d.package] || 0;
  total += CONCIERGE_PRICE[d.package]?.[d.conciergeCount] ?? 0;
  OPTIONS_LIST.forEach(opt => { if ((d.options || []).includes(opt.key)) total += opt.price; });
...
const getTabLabel = (tab, index, allTabs) => {
  ...
  if (tab.type === "端末入替") {
    return d.tenpoName ? `${d.tenpoName} - 端末入替` : "端末入替";
  }
...
// ==================== PDF共通 ====================
const LOGO_BASE64 = "data:image/jpeg;base64,";

const ps = {
  page:      { width: "210mm", minHeight: "297mm", padding: "10mm 14mm", boxSizing: "border-box", fontFamily: FONT, fontSize: "9pt", color: "#1a2535", background: "#fff" },
  secHeader: { fontSize: "9pt", fontWeight: 800, background: "#1a3358", color: "#fff", padding: "3px 8px", marginTop: "4mm", marginBottom: "1.5mm" },
  subHeader: { fontSize: "8.5pt", fontWeight: 700, color: "#1a3358", borderBottom: "1px solid #1a3358", paddingBottom: "1px", marginTop: "3mm", marginBottom: "1mm" },
  table:     { width: "100%", borderCollapse: "collapse", marginBottom: "1.5mm" },
  tdLabel:   { width: "36%", padding: "3px 7px", background: "#eef2f7", border: "1px solid #b0bec5", fontSize: "8pt", fontWeight: 600, verticalAlign: "top" },
  tdVal:     { padding: "3px 7px", border: "1px solid #b0bec5", fontSize: "8pt", verticalAlign: "top", whiteSpace: "pre-wrap" },
  moushikomType: { fontSize: "10.5pt", fontWeight: 800, color: "#1a3358", marginBottom: "2mm", paddingBottom: "1.5mm", borderBottom: "2px solid #1D9E75" },
  agreement:     { fontSize: "7.5pt", color: "#5f7490", lineHeight: 1.6, marginBottom: "1mm" },
};
...
const PdfHeader = ({ typeLabel, form }) => (
  <>
    ...
    <div style={{ display: "flex", gap: "6mm", alignItems: "flex-start", marginBottom: "2mm" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "2px", width: "55mm", flexShrink: 0 }}>
        {[["営業所", form.eigyosho], ["担当者", form.tantosha], ["社員番号", form.shainBango]].map(([l, v]) => (
          <div key={l} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: "8pt" }}>
            <span style={{ width: "50px", flexShrink: 0, color: "#5f7490" }}>{l}</span>
...
const PdfTenpoPage = ({ form, tab }) => {
  ...
  const typeLabel      = isTenpoAdd ? "新規申込（店舗追加）" : "新規申込（単店）";
  ...
  return (
    <div style={ps.page}>
      ...
      <table style={ps.table}><tbody>
        ...
        <TR label="オープン日"          value={getOpenDateLabel(d)} />
  ...
  return (
    <div style={ps.page}>
      ...
      <table style={ps.table}><tbody>
        <TR label="本部名"   value={d.honbuName} />
        <TR label="フリガナ" value={d.honbuNameKana} />
        <TR label="連携店舗" value={ld.tenpoName || "（未選択）"} />
      ...
      <table style={ps.table}><tbody>
        <tr>
          <td colSpan={2} style={{ ...ps.tdVal, color: "#5f7490", fontStyle: "italic" }}>
            ※ 連携店舗（{ld.tenpoName || "未選択"}）の日程と同じになります
          </td>
        </tr>
        {d.biko && <TR label="備考" value={d.biko} />}
      </tbody></table>
...
const Step3 = ({ form, onBack, onReset }) => {
  ...
  const downloadOnePdf = async (tab, idx) => {
    ...
    pdf.link(14, 34, 170, 5, { url: AGREEMENT_URL });
      ...
      return (
        <Card key={tab.id} style={{ marginBottom: 32 }}>
          ...
          <label style={{
            ...
            border: `1.5px solid ${isChecked ? C.teal : C.border}`,
