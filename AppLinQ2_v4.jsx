...
const ps = {
  ...
  subHeader: { background: "#dde4ef", padding: "2px 8px", fontSize: "8.5pt", fontWeight: 700, color: "#1a3358", marginBottom: "1mm", marginTop: "2mm" },
...
const PdfHeader = ({ typeLabel, form }) => (
  <>
    ...
    <div style={{ display: "flex", gap: "6mm", alignItems: "flex-end", marginBottom: "2mm" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "3px", width: "60mm", flexShrink: 0 }}>
        {[["営業所", form.eigyosho], ["担当者", form.tantosha], ["社員番号", form.shainBango]].map(([l, v]) => (
          <div key={l} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "8pt" }}>
            <span style={{ width: "46px", textAlign: "right", flexShrink: 0, color: "#5f7490" }}>{l}</span>
...
const PdfTenpoPage = ({ form, tab }) => {
  ...
  const hasWanHonbu = allTabs.some(t =>
    t.type === "新規_事務所WAN本部" ||
    t.type === "新規_フロントWAN本部_既存店" ||
    t.type === "新規_フロントWAN本部_新規店"
  );
  const tantoTabs  = allTabs.filter(t => t.type === "新規_単店");
  const tantoLabel = isTenpoAdd
    ? "新規申込（店舗追加）"
    : hasWanHonbu
      ? "新規申込（単店・WAN）"
      : tantoTabs.length >= 2
...
const Step3 = ({ form, onBack, onReset }) => {
  ...
  return (<>
    ...
    {activeTabs.map((tab, i) => {
      ...
      const dynamicTypeLabel = tab.type === "新規_単店"
        ? hasWanHonbu2 ? "新規申込（単店・WAN）"
          : tantoTabs2.length >= 2 ? "新規申込（単店・同一企業）"
          : "新規申込（単店）"
        : PDF_TYPE_LABEL[tab.type] || tab.type;
      const typeLabel   = dynamicTypeLabel;
