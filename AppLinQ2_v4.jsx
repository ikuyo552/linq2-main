...
const OPTIONS_LIST = [
  ...
  { key:"jiki",           label:"磁気カードシステム",price:500,       note:"" },
];
...
const WAN_CONCIERGE_MANAGER_OPTIONS = ["0台（申込なし）","1台","2台","3台","4台","5台"];
...
const TAB_GROUPS = [
  ...
  },
  {
    groupLabel: "契約変更",
    types: [
      { type: "契約変更_店舗",    shortLabel: "店舗",    subLabel: "パッケージ変更・オプション追加/解約" },
      { type: "契約変更_WAN本部", shortLabel: "WAN本部", subLabel: "オプション追加/解約" },
    ],
  },
  {
    groupLabel: "全解約",
    types: [
      { type: "全解約_店舗",    shortLabel: "店舗",    subLabel: "" },
      { type: "全解約_WAN本部", shortLabel: "WAN本部", subLabel: "" },
    ],
  },
];
...
const newNyuukaeHenkoData = () => ({
  ...
  shozai: false, mf: false, barcodePayment: false, barcode: false, milbon: false, jiki: false,
  rewrite: false,
  // 日程調整（常時表示）
  nicchoseiSaki: "",
  nicchoseiTantosha: "", nicchoseiTantoshaKana: "", nicchoseiDenwa: "",
...
// 契約変更（WAN本部）の初期データ
const newKeiyakuHenkoHonbuData = () => ({
  // 申込種別（パッケージ変更なし）
  moushikomiBetsu: { optionAdd: false, optionCancel: false },
  // 本部情報
  honbuName: "", honbuNameKana: "",
  sameAsKeiyakusha: false,
  yubinBango: "", jusho: "", jusho2: "", denwa: "",
  // 変更後のサービス（WAN本部専用）
  lanChecked: false, lanKeizoCount: 0, lanAddCount: 0, lanSetsuchiSha: "",
  wanConciergeManager: "",
  wanBarcode: false,
  wanShozai: false,
  // 手配（LAN追加あり→日程調整、なし→変更希望日）
  jizenchoosa: false, chosaDate: "",
  nicchoseiSaki: "", nicchoseiTantosha: "",
  nicchoseiTantoshaKana: "", nicchoseiDenwa: "",
  donguDate: "", nicchoseiHosoku: "",
...
const DATA_FN = {
  ...
  "契約変更_店舗":               newKeiyakuHenkoData,
  "契約変更_WAN本部":             newKeiyakuHenkoHonbuData,
  "全解約_店舗":                 newZenkaiData,
  "全解約_WAN本部":              newZenkaiData,
};
...
const getTabLabel = (tab, index, allTabs) => {
  ...
  if (tab.type === "契約変更_店舗") {
    return d.tenpoName ? `${d.tenpoName} - 契約変更（店舗）` : "契約変更（店舗）";
  }
  if (tab.type === "契約変更_WAN本部") {
    return d.honbuName ? `${d.honbuName} - 契約変更（WAN本部）` : "契約変更（WAN本部）";
  }
  if (tab.type === "全解約_店舗") {
    return d.tenpoName ? `${d.tenpoName} - 全解約（店舗）` : "全解約（店舗）";
  }
  if (tab.type === "全解約_WAN本部") {
    return d.honbuName ? `${d.honbuName} - 全解約（WAN本部）` : "全解約（WAN本部）";
  }
...
const UISelect = ({ children, style, hasError, ...props }) => (
  <select style={{ ...inputBase(hasError, false), cursor: "pointer", ...style }} {...props}>{children}</select>
);
...
const validateStep2 = (tabs) => {
  ...
  tabs.forEach(tab => {
    ...
    if (tab.type === "新規_単店" || tab.type === "新規_店舗追加") {
      ...
      if (d.netKaiTsubiStatus === "未開通" && d.netKaiTsubiKakutei === "確定") {
        if (d.netKaiTsubiMode === "date" && !d.netKaiTsubiDate) e.netKaiTsubiDate = "入力してください";
        if (d.netKaiTsubiMode === "text" && !d.netKaiTsubiText?.trim()) e.netKaiTsubiText = "入力してください";
      }
      if (!d.openStatus)           e.openStatus   = "選択してください";
      if (d.openStatus === "新規開業・出店" && d.openKakutei === "確定") {
        if (d.openMode === "date" && !d.openDate) e.openDate = "入力してください";
        if (d.openMode === "text" && !d.openText?.trim()) e.openText = "入力してください";
      }
    ...
    if (tab.type === "新規_フロントWAN本部_既存店") {
      ...
      if (!d.wanConciergeManager)       e.wanConciergeManager   = "選択してください";
      if (!d.kizonTenpoName?.trim())    e.kizonTenpoName        = "入力してください";
      if (!d.kizonTenpoDenwa)           e.kizonTenpoDenwa       = "入力してください";
      if (!d.nicchoseiSaki)             e.nicchoseiSaki         = "選択してください";
      if (!d.nicchoseiTantosha?.trim()) e.nicchoseiTantosha     = "入力してください";
    ...
    if (tab.type === "契約変更_店舗") {
      ...
      // 日程調整 or 変更希望日
      const needsVisitKhS = (d.ctiChecked && (d.ctiAddCount || 0) > 0) || (d.lanChecked && (d.lanAddCount || 0) > 0);
      if (needsVisitKhS) {
        if (!d.nicchoseiSaki)                 e.nicchoseiSaki         = "選択してください";
        if (!d.nicchoseiTantosha?.trim())     e.nicchoseiTantosha     = "入力してください";
        if (!d.nicchoseiTantoshaKana?.trim()) e.nicchoseiTantoshaKana = "入力してください";
    ...
    if (tab.type === "契約変更_WAN本部") {
      // 申込種別
      const mb = d.moushikomiBetsu || {};
      if (!mb.optionAdd && !mb.optionCancel)
        e._moushikomiBetsu = "申込種別を1つ以上選択してください";
      // 本部情報
      if (!d.honbuName?.trim())     e.honbuName     = "入力してください";
      if (!d.honbuNameKana?.trim()) e.honbuNameKana = "入力してください";
      ...
      if (!d.jusho2?.trim())        e.jusho2        = "入力してください";
      if (!d.denwa)                 e.denwa         = "入力してください";
      // Concierge for Manager
      if (!d.wanConciergeManager)   e.wanConciergeManager = "選択してください";
      // LAN 設置者
      if (d.lanChecked && (d.lanAddCount||0) > 0 && !d.lanSetsuchiSha) e.lanSetsuchiSha = "選択してください";
      // 手配（LAN追加台数>0なら日程調整必須、なければ変更希望日必須）
      const needsVisitKhH = d.lanChecked && (d.lanAddCount||0) > 0;
      if (needsVisitKhH) {
        if (!d.nicchoseiSaki)                 e.nicchoseiSaki         = "選択してください";
        if (!d.nicchoseiTantosha?.trim())     e.nicchoseiTantosha     = "入力してください";
        if (!d.nicchoseiTantoshaKana?.trim()) e.nicchoseiTantoshaKana = "入力してください";
...
const SelectBtn = ({ label, sub, disabled, onClick, done }) => {
  ...
  return (
    <button disabled={disabled} onClick={onClick}
      ...
      {subLines.map((line, i) => (
        <div key={i} style={{ fontSize:11, color:disabled?"#b0bec5":C.muted, marginTop:i===0?4:2, fontWeight:400, lineHeight:1.5 }}>{line}</div>
      ))}
    ...
    <Card>
      ...
      {!step1 && (
        <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
          ...
          <SelectBtn label="契約変更" sub="パッケージ・オプションの変更" onClick={() => setStep1("契約変更")} />
          <SelectBtn label="全解約" onClick={() => setStep1("全解約")} />
        </div>
      )}
      ...
      {step1 === "契約変更" && !step2 && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ fontSize:13.5, fontWeight:600, color:C.text, marginBottom:4 }}>店舗・WAN本部どちらの契約変更ですか？</div>
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            <SelectBtn label="🏪 店舗" sub={["パッケージ変更", "オプション追加/解約"]} onClick={() => onSelect("契約変更_店舗")} />
            <SelectBtn label="🏢 WAN本部" sub={["オプション追加/解約", "事務所WAN本部・フロントWAN本部、どちらも"]} onClick={() => onSelect("契約変更_WAN本部")} />
          </div>
        </div>
      )}
      ...
      {step1 === "全解約" && !step2 && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ fontSize:13.5, fontWeight:600, color:C.text, marginBottom:4 }}>店舗・WAN本部どちらの解約ですか？</div>
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            <SelectBtn label="🏪 店舗" onClick={() => onSelect("全解約_店舗")} />
            <SelectBtn label="🏢 WAN本部" sub="事務所WAN本部・フロントWAN本部、どちらも" onClick={() => onSelect("全解約_WAN本部")} />
          </div>
        </div>
      )}

      {/* 第2段階：店舗 or WAN本部（新規申込） */}
      {step1==="新規申込" && !step2 && (
        <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
          <SelectBtn label="🏪 店舗" onClick={() => setStep2("店舗")} />
          <SelectBtn label="🏢 WAN本部" onClick={() => setStep2("本部")} />
        </div>
      )}
    ...
    <Card>
      ...
      <Field label="オプション">
        <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
          ...
          <Field label="LinQ Concierge for Manager 台数" required error={te.wanConciergeManager}>
            <UISelect value={d.wanConciergeManager||""} hasError={!!te.wanConciergeManager} onChange={e => { upd("wanConciergeManager", e.target.value); clr("wanConciergeManager"); }} style={{ maxWidth:260 }}>
      ...
      <div style={{ background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:12, padding:"14px 18px", marginTop:8 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:4, fontSize:13 }}>
          ...
          {d.wanConciergeManager && d.wanConciergeManager!=="0台（申込なし）" && (() => {
      ...
      {d.package && (<>
        ...
        <Field label="オプション（複数選択可）">
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            ...
            {/* リライト（継続のみ・新規申込不可） */}
            <label style={optRow(d.rewrite)}>
              <input type="checkbox" checked={!!d.rewrite} style={{ width:16, height:16, cursor:"pointer" }}
                onChange={e => upd("rewrite", e.target.checked)} />
              <span style={{ fontSize:13, color:d.rewrite?C.tealDark:C.text, fontWeight:d.rewrite?600:400, flex:1 }}>
                リライト　<span style={{ fontSize:11, color:C.danger, fontWeight:400 }}>新規申込不可・継続のみ</span>
              </span>
              <span style={{ fontSize:12, color:C.muted }}>{fmtPrice(REWRITE_PRICE)}</span>
            </label>
          </div>
        </Field>
        ...
        {khTotal !== null && (
          <div style={{ background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:12, padding:"14px 18px", marginTop:8 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:4, fontSize:13 }}>
              ...
              {d.rewrite && <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>リライト</span><span>{fmtPrice(REWRITE_PRICE)}</span></div>}
              <div style={{ borderTop:`1px solid ${C.teal}`, marginTop:6, paddingTop:6, display:"flex", justifyContent:"space-between", fontWeight:800, fontSize:15 }}>
...
// 月額合計計算
const KH_OPT_KEY = { shozai:"shozai", mf:"mf", barcode_payment:"barcodePayment", barcode:"barcode", milbon:"milbon", jiki:"jiki" };
const calcKhTotal = (d) => {
  ...
  OPTIONS_LIST.forEach(o => { const k = KH_OPT_KEY[o.key]; if (k && d[k]) t += o.price; });
  if (d.rewrite) t += REWRITE_PRICE;
  return t;
};
...
// ==================== 契約変更（本部）フォーム ====================
const KeiyakuHenkoHonbuTabContent = ({ tab, form, updTab, clrTab, errors }) => {
  const d     = tab.data;
  const te    = errors || {};
  const tabId = tab.id;
  const upd   = (k, v) => updTab(tabId, k, v);
  ...
  const clr   = (k) => clrTab(tabId, k);
  const mb    = d.moushikomiBetsu || {};
  const needsVisit = d.lanChecked && (d.lanAddCount||0) > 0;
  ...
  // 月額合計（WAN本部専用）
  const wanMonthly = (() => {
    let t = WAN_HONBU_BASE;
    if (d.lanChecked) t += LAN_PRICE * ((d.lanKeizoCount||0) + (d.lanAddCount||0));
    const cIdx = WAN_CONCIERGE_MANAGER_OPTIONS.indexOf(d.wanConciergeManager);
    if (cIdx > 0) t += cIdx * WAN_CONCIERGE_MANAGER_PRICE;
    if (d.wanShozai) t += 1000;
    return t;
  })();
  ...
  return (<>
    ...
    <Card>
      ...
      <Field label="本部名" required error={te.honbuName}>
        <UIInput value={d.honbuName||""} hasError={!!te.honbuName}
          onChange={e => { upd("honbuName", e.target.value); clr("honbuName"); }} placeholder="例：〇〇本部" />
      </Field>
      <Field label="本部名フリガナ" required hint={!te.honbuNameKana?"手動入力（カタカナ）":undefined} error={te.honbuNameKana}>
        <UIInput value={d.honbuNameKana||""} hasError={!!te.honbuNameKana} onlyKatakana
          onChange={e => { upd("honbuNameKana", e.target.value); clr("honbuNameKana"); }} placeholder="例：マルマル ホンブ" />
      </Field>
      <div style={{ marginBottom:14 }}>
        <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontSize:13.5, fontWeight:500, color:C.text, userSelect:"none" }}>
          <input type="checkbox" checked={!!d.sameAsKeiyakusha}
      ...
      <ZipField value={d.yubinBango||""} onChange={v => { upd("yubinBango",v); clr("yubinBango"); }} onAddressFound={addr => { upd("jusho",addr); clr("jusho"); }} addressInputId={`kh-honbu-jusho2-${tabId}`} required errors={te} fieldKey="yubinBango" clearError={k => clr(k)} disabled={!!d.sameAsKeiyakusha} />
      <Field label="住所（自動入力）" error={te.jusho}>
        <UIInput value={d.jusho||""} hasError={!!te.jusho} disabled style={{ color:C.muted }} placeholder="郵便番号を入力すると自動入力されます" />
      </Field>
      <Field label="番地・建物名以降" required hint={!te.jusho2?"例：1-2-3　〇〇マンション101号室":undefined} error={te.jusho2}>
        <UIInput id={`kh-honbu-jusho2-${tabId}`} value={d.jusho2||""} hasError={!!te.jusho2} disabled={!!d.sameAsKeiyakusha}
          onChange={e => { upd("jusho2", e.target.value); clr("jusho2"); }} placeholder="例：1-2-3　〇〇マンション101号室" />
      </Field>
      <Field label="電話番号" required hint={!te.denwa?"ハイフンあり・なしどちらでも可":undefined} error={te.denwa}>
        <UIInput value={d.denwa||""} hasError={!!te.denwa} numHyphen disabled={!!d.sameAsKeiyakusha}
    ...
    <Card>
      ...
      <div style={{ fontSize:12.5, color:C.muted, marginBottom:10 }}>複数選択可</div>
      <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
        {[["optionAdd","オプション追加"],["optionCancel","オプション解約"]].map(([key,label]) => (
          <label key={key} style={{ display:"flex", alignItems:"center", gap:7, cursor:"pointer",
            padding:"8px 18px", borderRadius:22,
            border:`1.5px solid ${mb[key] ? C.teal : te._moushikomiBetsu ? C.danger : C.border}`,
            background:mb[key] ? C.tealLight : C.white,
    ...
    {/* サービス選択 */}
    <Card>
      <SecTitle icon="📦">変更後のサービス</SecTitle>
      <p style={{ fontSize:12.5, color:C.muted, marginBottom:14 }}>変更後の契約内容をすべて選択してください。月額合計が自動計算されます。</p>
      ...
      <Field label="オプション">
        <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
          ...
          {/* LAN */}
          <label style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", borderRadius:8, border:`1px solid ${d.lanChecked?C.teal:C.border}`, background:d.lanChecked?C.tealLight:C.white, cursor:"pointer" }}>
            <input type="checkbox" checked={!!d.lanChecked} style={{ width:16, height:16, cursor:"pointer" }} onChange={e => { upd("lanChecked", e.target.checked); if (!e.target.checked) updM({ lanKeizoCount:0, lanAddCount:0, lanSetsuchiSha:"" }); }} />
            <span style={{ fontSize:13, color:d.lanChecked?C.tealDark:C.text, fontWeight:d.lanChecked?600:400, flex:1 }}>LAN</span>
            <span style={{ fontSize:12, color:C.muted }}>{fmtPrice(LAN_PRICE)}/台</span>
          </label>
          {d.lanChecked && (
      ...
      <div style={{ background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:12, padding:"14px 18px", marginTop:8 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:4, fontSize:13 }}>
          <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>WAN本部</span><span>{fmtPrice(WAN_HONBU_BASE)}</span></div>
          {d.lanChecked && ((d.lanKeizoCount||0)+(d.lanAddCount||0)) > 0 && <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>LAN（{(d.lanKeizoCount||0)+(d.lanAddCount||0)}台）</span><span>{fmtPrice(LAN_PRICE*((d.lanKeizoCount||0)+(d.lanAddCount||0)))}</span></div>}
          {d.wanConciergeManager && d.wanConciergeManager!=="0台（申込なし）" && (() => {
            const idx = WAN_CONCIERGE_MANAGER_OPTIONS.indexOf(d.wanConciergeManager);
            return <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>Concierge for Manager（{d.wanConciergeManager}）</span><span>{fmtPrice(idx*WAN_CONCIERGE_MANAGER_PRICE)}</span></div>;
          })()}
...
const ZenkaiTabContent = ({ tab, form, updTab, clrTab, errors }) => {
  ...
  const te     = errors || {};
  const tabId  = tab.id;
  const isHonbu = tab.type === "全解約_WAN本部";
  const upd   = (k, v) => updTab(tabId, k, v);
  const updM  = (updates) => updTab(tabId, "__many__", updates);
  const clr   = (k)    => clrTab(tabId, k);

  const nameKey     = isHonbu ? "honbuName"     : "tenpoName";
  const nameKanaKey = isHonbu ? "honbuNameKana" : "tenpoNameKana";
  const nameLabel   = isHonbu ? "本部名"         : "店舗名";
  const nameKanaLabel = isHonbu ? "本部名フリガナ" : "店舗名フリガナ";
  const namePlaceholder = isHonbu ? "例：〇〇本部" : "例：ヘアサロン〇〇 渋谷店";
  const nameKanaPlaceholder = isHonbu ? "例：マルマル ホンブ" : "例：ヘアサロン マルマル シブヤテン";
  const sectionIcon  = isHonbu ? "🏢" : "🏪";
  const sectionLabel = isHonbu ? "本部情報" : "店舗情報";
  const jushoPrefix  = isHonbu ? "zenkai-honbu" : "zenkai";

  return (<>
    <Card>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18, paddingBottom:12, borderBottom:`2px solid ${C.border}` }}>
        <div style={{ display:"flex", alignItems:"center", gap:9 }}>
          <span style={{ fontSize:17 }}>{sectionIcon}</span>
          <h3 style={{ fontSize:15, fontWeight:700, color:C.navy }}>{sectionLabel}</h3>
        </div>
        <button onClick={() => { if (window.confirm("入力内容をリセットします。よいですか？")) updTab(tabId, "__reset__", null); }}
          style={{ fontSize:12, fontWeight:600, color:C.muted, background:C.white, border:`1.5px solid ${C.border}`, borderRadius:8, padding:"5px 12px", cursor:"pointer", fontFamily:FONT }}>
          🔄 リセット
      ...
      <Field label={nameLabel} required error={te[nameKey]}>
        <UIInput value={d[nameKey]||""} hasError={!!te[nameKey]}
          onChange={e => { upd(nameKey, e.target.value); clr(nameKey); if (!e.target.value) upd(nameKanaKey,""); }}
          placeholder={namePlaceholder} />
      </Field>
      <Field label={nameKanaLabel} required hint={!te[nameKanaKey] ? "手動入力（カタカナ）" : undefined} error={te[nameKanaKey]}>
        <UIInput value={d[nameKanaKey]||""} hasError={!!te[nameKanaKey]} onlyKatakana
          onChange={e => { upd(nameKanaKey, e.target.value); clr(nameKanaKey); }}
          placeholder={nameKanaPlaceholder} />
      </Field>
      ...
      <div style={{ marginBottom:14 }}>
        ...
        {d.sameAsKeiyakusha && (
          <div style={{ background:"#fef9c3", border:"1px solid #fde047", borderRadius:6, padding:"8px 12px", fontSize:12.5, color:"#854d0e", marginTop:6 }}>
            ⚠️ 契約者の住所・電話番号がコピーされます。{sectionLabel}として正しいか確認してください。
          </div>
        )}
      </div>

      <ZipField value={d.yubinBango||""} onChange={v => { upd("yubinBango",v); clr("yubinBango"); }} onAddressFound={addr => { upd("jusho",addr); clr("jusho"); }} addressInputId={`${jushoPrefix}-jusho2-${tabId}`} required errors={te} fieldKey="yubinBango" clearError={k => clr(k)} disabled={!!d.sameAsKeiyakusha} />
      <Field label="住所（自動入力）" error={te.jusho}>
        <UIInput value={d.jusho||""} hasError={!!te.jusho} disabled style={{ background:d.jusho?"#f0f4f8":undefined, color:C.muted }} placeholder="郵便番号を入力すると自動入力されます" />
      </Field>
      <Field label="番地・建物名以降" required hint={!te.jusho2 ? "例：1-2-3　〇〇マンション101号室" : undefined} error={te.jusho2}>
        <UIInput id={`${jushoPrefix}-jusho2-${tabId}`} value={d.jusho2||""} hasError={!!te.jusho2} disabled={!!d.sameAsKeiyakusha}
          onChange={e => { upd("jusho2", e.target.value); clr("jusho2"); }} placeholder="例：1-2-3　〇〇マンション101号室" />
      </Field>
      <Field label="電話番号" required hint={!te.denwa ? "ハイフンあり・なしどちらでも可" : undefined} error={te.denwa}>
        <UIInput value={d.denwa||""} hasError={!!te.denwa} numHyphen disabled={!!d.sameAsKeiyakusha}
            ...
            {OPTIONS_LIST.filter(o => o.key === "cti").map(opt => {
              ...
              return (
                <label key={opt.key} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", borderRadius:8, border:`1px solid ${checked?C.teal:C.border}`, background:checked?C.tealLight:C.white, cursor:"pointer", transition:"all .15s" }}>
                  ...
                  <span style={{ fontSize:12, color:C.muted }}>{d.package?fmtPrice(CTI_PRICE[d.package]||0):"¥0"}</span>
                </label>
              );
            })}
            {/* LAN（CTIの直後） */}
        ...
        : (() => {
            ...
            return (<>
              ...
              {(activeTab.type === "新規_単店" || activeTab.type === "新規_店舗追加")
                ...
                : activeTab.type === "全解約_店舗" || activeTab.type === "全解約_WAN本部"
                ? <ZenkaiTabContent tab={activeTab} form={form} updTab={updTab} clrTab={clrTab} errors={tabErrors} />
                : activeTab.type === "契約変更_店舗"
                ? <KeiyakuHenkoTabContent tab={activeTab} form={form} updTab={updTab} clrTab={clrTab} errors={tabErrors} />
                : activeTab.type === "契約変更_WAN本部"
                ? <KeiyakuHenkoHonbuTabContent tab={activeTab} form={form} updTab={updTab} clrTab={clrTab} errors={tabErrors} />
...
// ==================== PDF: 全解約 ====================
const PdfZenkaiPage = ({ form, tab }) => {
  const d = tab.data || {};
  const isHonbu = tab.type === "全解約_WAN本部";
  const nameLabel    = isHonbu ? "本部名"     : "店舗名";
  const nameKanaLabel = isHonbu ? "フリガナ"  : "フリガナ";
  const nameVal      = isHonbu ? d.honbuName  : d.tenpoName;
  const nameKanaVal  = isHonbu ? d.honbuNameKana : d.tenpoNameKana;
  const sectionLabel = isHonbu ? "本部情報"   : "店舗情報";
  const typeLabel    = isHonbu ? "全解約（WAN本部）" : "全解約（店舗）";
  ...
  return (
    <div style={ps.page}>
      ...
      <div style={ps.subHeader}>{sectionLabel}</div>
      <table style={ps.table}><tbody>
        <TR label={nameLabel}   value={nameVal} />
        <TR label={nameKanaLabel} value={nameKanaVal} />
        <TR label="郵便番号" value={d.yubinBango ? `〒${d.yubinBango}` : ""} />
        <TR label="住所"     value={[d.jusho, d.jusho2].filter(Boolean).join(" ")} />
        <TR label="電話番号" value={d.denwa} />
      </tbody></table>

      <div style={ps.subHeader}>解約情報</div>
      <table style={ps.table}><tbody>
        <TR label="解約希望日" value={d.kaiyakuKiboDate ? new Date(d.kaiyakuKiboDate).toLocaleDateString("ja-JP", { year:"numeric", month:"long", day:"numeric" }) : ""} />
        {d.biko && <TR label="備考" value={d.biko} />}
      </tbody></table>
...
// ==================== PDF: 契約変更（本部） ====================
const PdfKeiyakuHenkoHonbuPage = ({ form, tab }) => {
  const d  = tab.data || {};
  const mb = d.moushikomiBetsu || {};

  const mbLabels = [
    mb.optionAdd    && "オプション追加",
    mb.optionCancel && "オプション解約",
  ].filter(Boolean);
  const typeLabel = mbLabels.length > 0
    ? `契約変更（WAN本部）（${mbLabels.join(" / ")}）`
    : "契約変更（WAN本部）";
  ...
  return (
    <div style={ps.page}>
      ...
      <div style={ps.subHeader}>変更後のサービス　<span style={{ fontSize:"7pt", fontWeight:400, color:"#5f7490" }}>※ 月額・税別</span></div>
      <table style={ps.table}><tbody>
        <TR label="WAN本部" value={`¥${WAN_HONBU_BASE.toLocaleString()}（固定）`} />
        {d.lanChecked && ((d.lanKeizoCount||0)+(d.lanAddCount||0)) > 0 && (() => {
          const total = (d.lanKeizoCount||0)+(d.lanAddCount||0);
          const parts = [];
          if ((d.lanKeizoCount||0) > 0) parts.push(`継続${d.lanKeizoCount}台`);
          if ((d.lanAddCount||0) > 0)   parts.push(`追加${d.lanAddCount}台`);
          return <TR label="LAN" value={`${parts.join(" / ")}　¥${(LAN_PRICE*total).toLocaleString()}`} />;
        })()}
        {d.wanConciergeManager && d.wanConciergeManager !== "0台（申込なし）" && (() => {
          const idx = WAN_CONCIERGE_MANAGER_OPTIONS.indexOf(d.wanConciergeManager);
          return <TR label="Concierge for Manager" value={`${d.wanConciergeManager}　¥${(idx*WAN_CONCIERGE_MANAGER_PRICE).toLocaleString()}`} />;
        })()}
        {d.wanBarcode && <TR label="バーコード決済" value="¥0" />}
        {d.wanShozai  && <TR label="粧材管理"       value="¥1,000" />}
        <tr>
          <td style={{ ...ps.tdLabel, background:"#1a3358", color:"#fff", fontWeight:700 }}>月額合計（税別）</td>
          <td style={{ ...ps.tdVal, fontWeight:800, fontSize:"10pt" }}>¥{wanTotal.toLocaleString()}</td>
        </tr>
      </tbody></table>
      ...
      {needsVisit ? (<>
        ...
        <table style={ps.table}><tbody>
          ...
          {d.jizenchoosa && d.chosaDate && <TR label="調査希望日時" value={d.chosaDate} />}
          <TR label="導入希望日時"   value={d.donguDate} />
          {d.nicchoseiHosoku && <TR label="備考" value={d.nicchoseiHosoku} />}
        </tbody></table>
      </>) : (<>
        <div style={ps.subHeader}>変更希望日</div>
        <table style={ps.table}><tbody>
          <TR label="変更希望日" value={d.henkoKiboDate ? new Date(d.henkoKiboDate).toLocaleDateString("ja-JP", { year:"numeric", month:"long", day:"numeric" }) : ""} />
        </tbody></table>
      </>)}

      {d.biko && (<>
        <div style={ps.subHeader}>備考</div>
        <table style={ps.table}><tbody>
          <TR label="備考" value={d.biko} />
        </tbody></table>
      </>)}
...
// ==================== PDF: 契約変更 ====================
const PdfKeiyakuHenkoPage = ({ form, tab }) => {
  const d  = tab.data || {};
  const mb = d.moushikomiBetsu || {};

  // typeLabel を申込種別から動的生成
  const mbLabels = [
    mb.packageChange && "パッケージ変更",
    mb.optionAdd     && "オプション追加",
    mb.optionCancel  && "オプション解約",
  ].filter(Boolean);
  const typeLabel = mbLabels.length > 0
  ...
  // CTI/LAN 台数の表示文字列
  const fmtCounts = (keizoCount, addCount) => {
    const parts = [];
    if ((keizoCount||0) > 0) parts.push(`継続${keizoCount}台`);
    if ((addCount||0) > 0)   parts.push(`追加${addCount}台`);
    return parts.join(" / ");
  };
  ...
  // 訪問要否
  const needsVisit = ((d.ctiAddCount||0) > 0) || ((d.lanAddCount||0) > 0);
  ...
  return (
    <div style={ps.page}>
      ...
      <div style={ps.subHeader}>変更後のサービス　<span style={{ fontSize:"7pt", fontWeight:400, color:"#5f7490" }}>※ 月額・税別</span></div>
      <table style={ps.table}><tbody>
        <TR label="パッケージ" value={d.package ? `${d.package}　¥${(PACKAGE_BASE[d.package]||0).toLocaleString()}` : ""} />
        {d.concierge && (
          <TR label="LinQ Concierge"
            value={`${d.concierge}${CONCIERGE_NEEDS_ACTUAL.includes(d.concierge) && d.conciergeActual ? `（実${d.conciergeActual}台）` : ""}　¥${(CONCIERGE_PRICE[d.package]?.[d.concierge]??0).toLocaleString()}`} />
        )}
        {d.ctiChecked && ((d.ctiKeizoCount||0)+(d.ctiAddCount||0)) > 0 && (
          <TR label="CTI" value={`${fmtCounts(d.ctiKeizoCount, d.ctiAddCount)}　¥${(CTI_PRICE[d.package]||0).toLocaleString()}${(d.ctiAddCount||0) > 0 && d.ctiSetsuchiSha ? `（設置者：${d.ctiSetsuchiSha}）` : ""}`} />
        )}
        {d.lanChecked && ((d.lanKeizoCount||0)+(d.lanAddCount||0)) > 0 && (
          <TR label="LAN" value={`${fmtCounts(d.lanKeizoCount, d.lanAddCount)}　¥${(((d.lanKeizoCount||0)+(d.lanAddCount||0))*LAN_PRICE).toLocaleString()}${(d.lanAddCount||0) > 0 && d.lanSetsuchiSha ? `（設置者：${d.lanSetsuchiSha}）` : ""}`} />
        )}
        {(() => {
          const selected = [
            ...OPTIONS_LIST.filter(o => o.key !== "cti" && d[KH_OPT_KEY[o.key]]),
            d.rewrite && { label: "リライト", price: REWRITE_PRICE },
          ].filter(Boolean);
          return selected.length > 0
            ? <TR label="オプション" value={selected.map(o => `${o.label}　¥${o.price.toLocaleString()}`).join("　／　")} />
            : null;
        })()}
        {khTotal !== null && (
          <tr>
            <td style={{ ...ps.tdLabel, background:"#1a3358", color:"#fff", fontWeight:700 }}>月額合計（税別）</td>
            <td style={{ ...ps.tdVal, fontWeight:800, fontSize:"10pt" }}>¥{khTotal.toLocaleString()}</td>
          </tr>
        )}
      </tbody></table>
      ...
      {(d.bikoKaiyaku || d.biko) && (<>
        <div style={ps.subHeader}>備考</div>
        <table style={ps.table}><tbody>
          {d.bikoKaiyaku && <TR label="解約サービスについて" value={d.bikoKaiyaku} />}
          {d.biko        && <TR label="備考"                 value={d.biko} />}
        </tbody></table>
      </>)}
...
// ==================== PDF: 端末入替（変更あり） ====================
const PdfNyuukaeHenkoPage = ({ form, tab }) => {
  const d = tab.data || {};
  ...
  return (
    <div style={ps.page}>
      <PdfHeader typeLabel="端末入替（変更あり）" form={form} />
      ...
      <table style={ps.table}><tbody>
        ...
        {d.ctiChecked && ((d.ctiKeizoCount||0)+(d.ctiAddCount||0)) > 0 && (
          <TR label="CTI" value={`${fmtCounts(d.ctiKeizoCount, d.ctiAddCount)}　¥${(CTI_PRICE[d.package]||0).toLocaleString()}`} />
        )}
        {d.lanChecked && ((d.lanKeizoCount||0)+(d.lanAddCount||0)) > 0 && (
          <TR label="LAN" value={`${fmtCounts(d.lanKeizoCount, d.lanAddCount)}　¥${(((d.lanKeizoCount||0)+(d.lanAddCount||0))*LAN_PRICE).toLocaleString()}`} />
        )}
        {(() => {
          const selected = [
            ...OPTIONS_LIST.filter(o => o.key !== "cti" && d[KH_OPT_KEY[o.key]]),
...
const PDF_COMPONENT = {
  ...
  "端末入替":                    PdfNyuukaePage,
  "端末入替_変更あり":           PdfNyuukaeHenkoPage,
  "全解約_店舗":                 PdfZenkaiPage,
  "全解約_WAN本部":               PdfZenkaiPage,
  "契約変更_店舗":               PdfKeiyakuHenkoPage,
  "契約変更_WAN本部":            PdfKeiyakuHenkoHonbuPage,
};
...
const getTabDisplayName = (tab) => {
  ...
  if (tab.type === "端末入替_変更あり") return d.tenpoName || "（店舗名未入力）";
  if (tab.type === "全解約_店舗")      return d.tenpoName  || "（店舗名未入力）";
  if (tab.type === "全解約_WAN本部")   return d.honbuName  || "（本部名未入力）";
  if (tab.type === "契約変更_店舗")    return d.tenpoName  || "（店舗名未入力）";
  if (tab.type === "契約変更_WAN本部") return d.honbuName  || "（本部名未入力）";
