...
const newKeiyakuHenkoData = () => ({
  ...
  concierge: "",
  conciergeActual: "",
  ctiChecked: false, ctiStatus: "",   // ctiStatus: "継続" / "新たに導入"
  ctiSetsuchiSha: "",
  lanChecked: false, lanStatus: "",   // lanStatus: "継続" / "新たに導入"
  lanCount: 1,
  lanSetsuchiSha: "",
...
const validateStep2 = (tabs) => {
  ...
  tabs.forEach(tab => {
    ...
    if (tab.type === "契約変更") {
      ...
      // 設置者・状態
      if (d.ctiChecked && !d.ctiStatus)                              e.ctiStatus     = "選択してください";
      if (d.ctiChecked && d.ctiStatus === "新たに導入" && !d.ctiSetsuchiSha) e.ctiSetsuchiSha = "選択してください";
      if (d.lanChecked && !d.lanStatus)                              e.lanStatus     = "選択してください";
      if (d.lanChecked && d.lanStatus === "新たに導入" && !d.lanSetsuchiSha) e.lanSetsuchiSha = "選択してください";
      // 日程調整 or 変更希望日
      const needsVisit = (d.ctiChecked && d.ctiStatus === "新たに導入") || (d.lanChecked && d.lanStatus === "新たに導入");
      if (needsVisit) {
        if (!d.nicchoseiSaki)                 e.nicchoseiSaki         = "選択してください";
        if (!d.nicchoseiTantosha?.trim())     e.nicchoseiTantosha     = "入力してください";
        if (!d.nicchoseiTantoshaKana?.trim()) e.nicchoseiTantoshaKana = "入力してください";
...
const calcKhTotal = (d) => {
  ...
  if (d.lanChecked && d.lanStatus && d.lanStatus !== "なし") t += (d.lanCount || 1) * LAN_PRICE;
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            ...
            <label style={optRow(d.ctiChecked)}>
              <input type="checkbox" checked={!!d.ctiChecked} style={{ width:16, height:16, cursor:"pointer" }}
                onChange={e => { upd("ctiChecked", e.target.checked); if (!e.target.checked) { upd("ctiStatus",""); upd("ctiSetsuchiSha",""); } }} />
            ...
            {d.ctiChecked && (
              <div style={{ paddingLeft:36, marginTop:2, marginBottom:4 }}>
                <div style={{ fontSize:12, color:C.muted, marginBottom:6 }}>導入状況</div>
                <div style={{ display:"flex", gap:8 }}>
                  {["継続","新たに導入"].map(v => (
                    <label key={v} style={{ display:"flex", alignItems:"center", gap:5, cursor:"pointer", padding:"5px 13px", borderRadius:8,
                      border:`1.5px solid ${d.ctiStatus===v ? C.teal : te.ctiStatus ? C.danger : C.border}`,
                      background:d.ctiStatus===v?C.tealLight:C.white, fontSize:13, fontWeight:d.ctiStatus===v?600:400 }}>
                      <input type="radio" checked={d.ctiStatus===v} onChange={() => { upd("ctiStatus",v); if (v!=="新たに導入") upd("ctiSetsuchiSha",""); clr("ctiStatus"); }}
                        style={{ width:13, height:13, cursor:"pointer", accentColor:C.teal }} />
                      {v}
                    </label>
                  ))}
                </div>
                {te.ctiStatus && <div style={{ fontSize:12, color:C.danger, marginTop:3 }}>⚠ {te.ctiStatus}</div>}
                {d.ctiStatus==="新たに導入" && (
                  <div style={{ marginTop:8 }}>
                    <div style={{ fontSize:12, color:C.muted, marginBottom:5 }}>設置者</div>
                    <div style={{ display:"flex", gap:8 }}>
            ...
            <label style={optRow(d.lanChecked)}>
              <input type="checkbox" checked={!!d.lanChecked} style={{ width:16, height:16, cursor:"pointer" }}
                onChange={e => { upd("lanChecked", e.target.checked); if (!e.target.checked) { upd("lanStatus",""); upd("lanSetsuchiSha",""); } }} />
              <span style={{ fontSize:13, color:d.lanChecked?C.tealDark:C.text, fontWeight:d.lanChecked?600:400, flex:1 }}>LANオプション</span>
              <span style={{ fontSize:12, color:C.muted }}>{fmtPrice(LAN_PRICE)}/台</span>
            </label>
            {d.lanChecked && (
              <div style={{ paddingLeft:36, marginTop:2, marginBottom:4 }}>
                ...
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  {["継続","新たに導入"].map(v => (
                    <label key={v} style={{ display:"flex", alignItems:"center", gap:5, cursor:"pointer", padding:"5px 13px", borderRadius:8,
                      border:`1.5px solid ${d.lanStatus===v ? C.teal : te.lanStatus ? C.danger : C.border}`,
                      background:d.lanStatus===v?C.tealLight:C.white, fontSize:13, fontWeight:d.lanStatus===v?600:400 }}>
                      <input type="radio" checked={d.lanStatus===v} onChange={() => { upd("lanStatus",v); if (v!=="新たに導入") upd("lanSetsuchiSha",""); clr("lanStatus"); }}
                        style={{ width:13, height:13, cursor:"pointer", accentColor:C.teal }} />
                      {v}
                    </label>
                  ))}
                </div>
                {te.lanStatus && <div style={{ fontSize:12, color:C.danger, marginTop:3 }}>⚠ {te.lanStatus}</div>}
                {d.lanStatus && (
        ...
        {khTotal !== null && (
          <div style={{ background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:12, padding:"14px 18px", marginTop:8 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:4, fontSize:13 }}>
              ...
              {d.ctiChecked && <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>CTI</span><span>{fmtPrice(CTI_PRICE[d.package]||0)}</span></div>}
              ...
              {d.lanChecked && d.lanStatus && <div style={{ display:"flex", justifyContent:"space-between" }}><span style={{ color:C.muted }}>LAN（{d.lanCount||1}台）</span><span>{fmtPrice((d.lanCount||1)*LAN_PRICE)}</span></div>}
