...
const newKeiyakuHenkoData = () => ({
  ...
  concierge: "",
  conciergeActual: "",
  ctiStatus: "",        // "" / "なし" / "継続" / "新たに導入"
  ctiSetsuchiSha: "",
  lanStatus: "",        // "" / "なし" / "継続" / "新たに導入"
  lanCount: 1,
  lanSetsuchiSha: "",
...
const validateStep2 = (tabs) => {
  ...
  tabs.forEach(tab => {
    ...
    if (tab.type === "契約変更") {
      ...
      // 設置者
      if (d.ctiStatus === "新たに導入" && !d.ctiSetsuchiSha) e.ctiSetsuchiSha = "選択してください";
      if (d.lanStatus === "新たに導入" && !d.lanSetsuchiSha) e.lanSetsuchiSha = "選択してください";
      // 日程調整 or 変更希望日
      const needsVisit = d.ctiStatus === "新たに導入" || d.lanStatus === "新たに導入";
      if (needsVisit) {
        if (!d.nicchoseiSaki)                 e.nicchoseiSaki         = "選択してください";
        if (!d.nicchoseiTantosha?.trim())     e.nicchoseiTantosha     = "入力してください";
        if (!d.nicchoseiTantoshaKana?.trim()) e.nicchoseiTantoshaKana = "入力してください";
...
const calcKhTotal = (d) => {
  ...
  if (d.lanStatus && d.lanStatus !== "なし") t += (d.lanCount || 1) * LAN_PRICE;
...
const KeiyakuHenkoTabContent = ({ tab, form, updTab, clrTab, errors }) => {
  ...
  const svcLabel = { fontSize:13.5, fontWeight:600, color:C.text, minWidth:120 };
  const svcPrice = { fontSize:12, color:C.muted, textAlign:"right", minWidth:90, paddingTop:2 };

  // 3択ラジオ（なし/継続/新たに導入）
  const StatusRadio = ({ value, onChange, options, error }) => (
    <div>
      <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
        {options.map(opt => (
          <label key={opt} style={{ display:"flex", alignItems:"center", gap:5, cursor:"pointer",
            padding:"5px 13px", borderRadius:8,
            border:`1.5px solid ${value===opt ? C.teal : error ? C.danger : C.border}`,
            background:value===opt ? C.tealLight : C.white,
            fontSize:13, fontWeight:value===opt?600:400, color:value===opt?C.tealDark:C.text }}>
  ...
  return (<>
    ...
    {/* サービス選択 */}
    <Card>
      <SecTitle icon="🔧">今後のサービス内容を選択してください</SecTitle>
      <div style={{ fontSize:12.5, color:C.muted, marginBottom:14 }}>申込後の契約内容をすべて選択してください。月額合計が自動計算されます。</div>

      {/* パッケージ */}
      <div style={svcRow(false)}>
        <div style={{ flex:1 }}>
          <div style={{ ...svcLabel, marginBottom:8 }}>パッケージ <span style={{ color:C.danger }}>*</span></div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {PACKAGES.map(p => (
              <label key={p} style={{ display:"flex", alignItems:"center", gap:6, cursor:"pointer",
                padding:"7px 16px", borderRadius:8,
                border:`1.5px solid ${d.package===p ? C.teal : te.package ? C.danger : C.border}`,
                background:d.package===p ? C.tealLight : C.white,
                fontSize:13, fontWeight:d.package===p?600:400, color:d.package===p?C.tealDark:C.text }}>
                <input type="radio" checked={d.package===p}
                  onChange={() => { upd("package",p); upd("concierge",""); upd("conciergeActual",""); clr("package"); clr("concierge"); }}
                  style={{ width:13, height:13, cursor:"pointer", accentColor:C.teal }} />
                {p}
              </label>
            ))}
          </div>
          {te.package && <div style={{ fontSize:12, color:C.danger, marginTop:4 }}>⚠ {te.package}</div>}
        </div>
        <div style={svcPrice}>{d.package ? fmtPrice(PACKAGE_BASE[d.package])+"/月" : "—"}</div>
      </div>

      {/* コンシェルジュ */}
      <div style={svcRow(false)}>
        <div style={{ flex:1 }}>
          <div style={{ ...svcLabel, marginBottom:8 }}>コンシェルジュ <span style={{ color:C.danger }}>*</span></div>
          {d.package ? (
            <>
              <UISelect value={d.concierge||""} hasError={!!te.concierge}
                ...
                {(CONCIERGE_BANDS[d.package]||[]).map(o => (
                  <option key={o} value={o}>{o}　{fmtPrice(CONCIERGE_PRICE[d.package]?.[o]||0)}/月</option>
                ))}
              </UISelect>
              {te.concierge && <div style={{ fontSize:12, color:C.danger, marginTop:4 }}>⚠ {te.concierge}</div>}
            ...
            <div style={{ fontSize:12.5, color:C.muted }}>先にパッケージを選択してください</div>
          )}
        </div>
        <div style={svcPrice}>{d.concierge && d.package && CONCIERGE_PRICE[d.package] ? fmtPrice(CONCIERGE_PRICE[d.package][d.concierge]||0)+"/月" : "—"}</div>
      </div>

      {/* CTI */}
      <div style={svcRow(false)}>
        <div style={{ flex:1 }}>
          <div style={{ ...svcLabel, marginBottom:8 }}>CTI</div>
          <StatusRadio value={d.ctiStatus} options={CTI_LAN_STATUS}
            onChange={v => { upd("ctiStatus",v); if (v !== "新たに導入") upd("ctiSetsuchiSha",""); }}
            error={te.ctiStatus} />
          {d.ctiStatus === "新たに導入" && (
            <SetsuchiShaField value={d.ctiSetsuchiSha}
              onChange={v => { upd("ctiSetsuchiSha",v); clr("ctiSetsuchiSha"); }}
              error={te.ctiSetsuchiSha} />
          )}
        </div>
        <div style={svcPrice}>¥0/月</div>
      </div>

      {/* LAN */}
      <div style={svcRow(false)}>
        <div style={{ flex:1 }}>
          <div style={{ ...svcLabel, marginBottom:8 }}>LANオプション</div>
          <StatusRadio value={d.lanStatus} options={CTI_LAN_STATUS}
            onChange={v => { upd("lanStatus",v); if (v === "なし") { upd("lanCount",1); upd("lanSetsuchiSha",""); } if (v !== "新たに導入") upd("lanSetsuchiSha",""); }}
            error={te.lanStatus} />
          {d.lanStatus && d.lanStatus !== "なし" && (
            <div style={{ marginTop:8, display:"flex", alignItems:"center", gap:10 }}>
              <span style={{ fontSize:12.5, color:C.muted }}>ポート数</span>
              <UISelect value={d.lanCount||1} onChange={e => upd("lanCount", Number(e.target.value))} style={{ maxWidth:160 }}>
                {LAN_COUNT_OPTIONS.map(n => <option key={n} value={n}>{n}台　{fmtPrice(n*LAN_PRICE)}/月</option>)}
              </UISelect>
            </div>
          )}
          {d.lanStatus === "新たに導入" && (
            <SetsuchiShaField value={d.lanSetsuchiSha}
              onChange={v => { upd("lanSetsuchiSha",v); clr("lanSetsuchiSha"); }}
              error={te.lanSetsuchiSha} />
          )}
        </div>
        <div style={svcPrice}>{d.lanStatus && d.lanStatus !== "なし" ? fmtPrice((d.lanCount||1)*LAN_PRICE)+"/月" : "—"}</div>
      </div>

      {/* オプション各種 */}
      {OPTIONS_LIST.filter(o => o.key !== "cti").map((opt, idx, arr) => {
        const dataKey = KH_OPT_KEY[opt.key];
        return (
          <div key={opt.key} style={svcRow(idx === arr.length - 1)}>
            <div style={{ flex:1 }}>
              <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", userSelect:"none" }}>
                <input type="checkbox" checked={!!d[dataKey]}
                  onChange={e => upd(dataKey, e.target.checked)}
                  style={{ width:14, height:14, cursor:"pointer", accentColor:C.teal }} />
                <span style={{ ...svcLabel, marginBottom:0, fontWeight:d[dataKey]?700:600, color:d[dataKey]?C.tealDark:C.text }}>{opt.label}</span>
              </label>
            </div>
            <div style={svcPrice}>{opt.price ? fmtPrice(opt.price)+"/月" : "¥0/月"}</div>
          </div>
        );
      })}

      {/* リライト・磁気（解約専用） */}
      <div style={{ marginTop:16, paddingTop:12, borderTop:`1.5px dashed ${C.border}` }}>
        <div style={{ fontSize:12, color:C.muted, marginBottom:10 }}>継続・解約される場合は選択してください</div>
        {[["rewrite","リライト",REWRITE_PRICE],["jiki","磁気",JIKI_PRICE]].map(([key,label,price]) => (
          <div key={key} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10, gap:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:12, flex:1 }}>
              <span style={{ fontSize:13.5, fontWeight:600, color:C.text, minWidth:60 }}>{label}</span>
              <UISelect value={d[key]||""} onChange={e => upd(key, e.target.value)} style={{ maxWidth:160 }}>
                <option value="">選択なし</option>
                {REWRITE_JIKI_STATUS.map(s => <option key={s} value={s}>{s}</option>)}
              </UISelect>
            </div>
            <div style={svcPrice}>{d[key]==="継続" ? fmtPrice(price)+"/月" : d[key]==="解約" ? "解約" : "—"}</div>
          </div>
        ))}
      </div>
      ...
      {/* 月額合計 */}
      {khTotal !== null && (
        <div style={{ marginTop:16, padding:"14px 18px", background:C.tealLight, borderRadius:10,
          border:`2px solid ${C.teal}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <span style={{ fontSize:14, fontWeight:700, color:C.tealDark }}>変更後の月額合計（税抜）</span>
          <span style={{ fontSize:20, fontWeight:800, color:C.tealDark }}>{fmtPrice(khTotal)}<span style={{ fontSize:13, fontWeight:600 }}>/月</span></span>
        </div>
      )}
    </Card>
