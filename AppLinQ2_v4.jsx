...
const TAB_GROUPS = [
  ...
  },
  {
    groupLabel: "契約変更",
    types: [
      { type: "契約変更_店舗", shortLabel: "店舗", subLabel: "パッケージ・オプションの追加・変更・解約" },
      { type: "契約変更_本部", shortLabel: "本部", subLabel: "本部オプションの追加・解約" },
    ],
  },
  {
    groupLabel: "全解約",
    types: [
      { type: "全解約_店舗", shortLabel: "店舗", subLabel: "サービスを解約します" },
      { type: "全解約_本部", shortLabel: "本部", subLabel: "本部サービスを解約します" },
    ],
  },
];
...
const DATA_FN = {
  ...
  "契約変更_本部":               newKeiyakuHenkoHonbuData,
  "全解約_店舗":                 newZenkaiData,
  "全解約_本部":                 newZenkaiData,
};
...
const getTabLabel = (tab, index, allTabs) => {
  ...
  if (tab.type === "契約変更_本部") {
    return d.honbuName ? `${d.honbuName} - 契約変更（本部）` : "契約変更（本部）";
  }
  if (tab.type === "全解約_店舗") {
    return d.tenpoName ? `${d.tenpoName} - 全解約（店舗）` : "全解約（店舗）";
  }
  if (tab.type === "全解約_本部") {
    return d.honbuName ? `${d.honbuName} - 全解約（本部）` : "全解約（本部）";
  }
...
const TypeSelector = ({ onSelect, form, onJump }) => {
  ...
  return (
    <Card>
      ...
      {step1 === "契約変更" && !step2 && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ fontSize:13.5, fontWeight:600, color:C.text, marginBottom:4 }}>店舗・本部どちらの契約変更ですか？</div>
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            <SelectBtn label="🏪 店舗" sub="パッケージ・オプションの追加・変更・解約" onClick={() => onSelect("契約変更_店舗")} />
            <SelectBtn label="🏢 本部" sub="本部オプションの追加・解約" onClick={() => onSelect("契約変更_本部")} />
          </div>
        </div>
      )}
      ...
      {step1 === "全解約" && !step2 && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ fontSize:13.5, fontWeight:600, color:C.text, marginBottom:4 }}>店舗・本部どちらの解約ですか？</div>
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            <SelectBtn label="🏪 店舗" sub="サービスを解約します" onClick={() => onSelect("全解約_店舗")} />
            <SelectBtn label="🏢 本部" sub="本部サービスを解約します" onClick={() => onSelect("全解約_本部")} />
          </div>
        </div>
      )}
        ...
        : (() => {
            ...
            return (<>
              ...
              {(activeTab.type === "新規_単店" || activeTab.type === "新規_店舗追加")
                ...
                : activeTab.type === "全解約_店舗" || activeTab.type === "全解約_本部"
...
const PdfZenkaiPage = ({ form, tab }) => {
  ...
  const sectionLabel = isHonbu ? "本部情報"   : "店舗情報";
  const typeLabel    = isHonbu ? "全解約（本部）" : "全解約（店舗）";
...
const PDF_COMPONENT = {
  ...
  "全解約_本部":                 PdfZenkaiPage,
  "契約変更_店舗":               PdfKeiyakuHenkoPage,
  "契約変更_本部":               PdfKeiyakuHenkoHonbuPage,
};
...
const getTabDisplayName = (tab) => {
  ...
  if (tab.type === "全解約_本部")    return d.honbuName  || "（本部名未入力）";
  if (tab.type === "契約変更_店舗")  return d.tenpoName  || "（店舗名未入力）";
  if (tab.type === "契約変更_本部")  return d.honbuName  || "（本部名未入力）";
