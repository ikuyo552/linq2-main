...
const newKeiyakuHenkoData = () => ({
  ...
  yubinBango: "", jusho: "", jusho2: "", denwa: "",
  // 追加・変更
  addPackage: false, packageAfter: "",
  addConcierge: false, addConciergeCount: "",
  addLan: false, addLanSetsuchiSha: "",
  addCti: false, addCtiSetsuchiSha: "",
  addShozai: false, addMfCloud: false,
  addBarcodePay: false, addBarcode: false, addMilbon: false,
  // 解約
  cancelConcierge: false, cancelConciergeRejiNo: "",
  cancelLan: false, cancelLanRejiNo: "",
  cancelCti: false, cancelShozai: false,
  cancelMfCloud: false, cancelBarcodePay: false,
  cancelBarcode: false, cancelMilbon: false,
  cancelRewrite: false, cancelRewriteRejiNo: "",
  cancelJiki: false, cancelJikiRejiNo: "",
  // 日程調整（CTI/LAN追加時）
...
const validateStep2 = (tabs) => {
  ...
  tabs.forEach(tab => {
    ...
    if (tab.type === "端末入替" || tab.type === "端末入替_変更あり") {
      // 店舗情報
      if (!d.tenpoName?.trim())     e.tenpoName     = "入力してください";
      if (!d.tenpoNameKana?.trim()) e.tenpoNameKana = "入力してください";
      if (!d.yubinBango)            e.yubinBango    = "入力してください";
    ...
    if (tab.type === "契約変更") {
      ...
      if (!d.denwa)                 e.denwa         = "入力してください";
      // 追加・解約どちらか1つ以上必須
      const hasAnyChange = d.addPackage||d.addConcierge||d.addLan||d.addCti||d.addShozai||d.addMfCloud||d.addBarcodePay||d.addBarcode||d.addMilbon||d.cancelConcierge||d.cancelLan||d.cancelCti||d.cancelShozai||d.cancelMfCloud||d.cancelBarcodePay||d.cancelBarcode||d.cancelMilbon||d.cancelRewrite||d.cancelJiki;
      if (!hasAnyChange) e._noChange = "追加・変更または解約する内容を1つ以上選択してください";
      // 追加項目の詳細
      if (d.addPackage  && !d.packageAfter)         e.packageAfter        = "選択してください";
      if (d.addConcierge && !d.addConciergeCount)   e.addConciergeCount   = "選択してください";
      if (d.addLan      && !d.addLanSetsuchiSha)    e.addLanSetsuchiSha   = "選択してください";
      if (d.addCti      && !d.addCtiSetsuchiSha)    e.addCtiSetsuchiSha   = "選択してください";
      // 解約項目のレジNO
      if (d.cancelConcierge && !d.cancelConciergeRejiNo?.trim()) e.cancelConciergeRejiNo = "入力してください";
      if (d.cancelLan    && !d.cancelLanRejiNo?.trim())   e.cancelLanRejiNo   = "入力してください";
      if (d.cancelRewrite && !d.cancelRewriteRejiNo?.trim()) e.cancelRewriteRejiNo = "入力してください";
      if (d.cancelJiki   && !d.cancelJikiRejiNo?.trim())  e.cancelJikiRejiNo  = "入力してください";
      // 日程調整（CTI/LAN追加時）
      const needsVisit = d.addCti || d.addLan;
      if (needsVisit) {
        if (!d.nicchoseiSaki)               e.nicchoseiSaki         = "選択してください";
        if (!d.nicchoseiTantosha?.trim())   e.nicchoseiTantosha     = "入力してください";
        if (!d.nicchoseiTantoshaKana?.trim()) e.nicchoseiTantoshaKana = "入力してください";
...
const CONCIERGE_ADD_OPTIONS = ["1台","2台","3台","4台","5台"];

// チェックアイテム共通スタイル
const ChkRow = ({ checked, label, onChange, children, error }) => (
  <div style={{ marginBottom: children || error ? 10 : 6 }}>
    <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", userSelect:"none",
      padding:"9px 14px", borderRadius:10,
      border:`1.5px solid ${checked ? C.teal : error ? C.danger : C.border}`,
      background: checked ? C.tealLight : error ? "#fff5f5" : C.white,
      transition:"all .15s" }}>
      <input type="checkbox" checked={!!checked} onChange={onChange}
        style={{ width:15, height:15, cursor:"pointer", accentColor:C.teal }} />
      <span style={{ fontSize:13.5, fontWeight:checked?600:400, color:checked?C.tealDark:C.text }}>{label}</span>
    </label>
    {checked && children && (
      <div style={{ marginTop:6, paddingLeft:16, borderLeft:`3px solid ${C.teal}` }}>
        {children}
      </div>
    )}
    {error && !checked && <div style={{ fontSize:12, color:C.danger, marginTop:3, paddingLeft:4 }}>⚠ {error}</div>}
  </div>
);

const SetsuchiShaField = ({ value, onChange, error }) => (
  <div style={{ marginBottom:6 }}>
    <div style={{ fontSize:12.5, color:C.muted, marginBottom:5 }}>設置者</div>
    <div style={{ display:"flex", gap:10 }}>
      {SETSUCHI_SHA.map(v => (
        <label key={v} style={{ display:"flex", alignItems:"center", gap:6, cursor:"pointer",
          ...
          background: value===v ? C.tealLight : C.white, fontSize:13, fontWeight:value===v?600:400,
          color: value===v ? C.tealDark : C.text }}>
...
const KeiyakuHenkoTabContent = ({ tab, form, updTab, clrTab, errors }) => {
  ...
  return (<>
    ...
    <Card>
      ...
      <Field label="店舗名" required error={te.tenpoName}>
        <UIInput value={d.tenpoName||""} hasError={!!te.tenpoName}
          onChange={e => { upd("tenpoName", e.target.value); clr("tenpoName"); }}
          placeholder="例：ヘアサロン〇〇 渋谷店" />
      </Field>
      <Field label="店舗名フリガナ" required hint={!te.tenpoNameKana ? "手動入力（カタカナ）" : undefined} error={te.tenpoNameKana}>
        <UIInput value={d.tenpoNameKana||""} hasError={!!te.tenpoNameKana} onlyKatakana
    ...
    {/* 追加・変更する内容 */}
    <Card>
      <SecTitle icon="➕">追加・変更する内容</SecTitle>
      {te._noChange && (
      ...
      <ChkRow checked={d.addPackage} label="パッケージ変更"
        onChange={e => { upd("addPackage", e.target.checked); if (!e.target.checked) upd("packageAfter",""); clr("_noChange"); }}>
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", paddingTop:4 }}>
          {PACKAGES.map(p => (
            <label key={p} style={{ display:"flex", alignItems:"center", gap:6, cursor:"pointer",
              padding:"6px 16px", borderRadius:8,
              border:`1.5px solid ${d.packageAfter===p ? C.teal : te.packageAfter ? C.danger : C.border}`,
              background: d.packageAfter===p ? C.tealLight : C.white, fontSize:13, fontWeight:d.packageAfter===p?600:400 }}>
              <input type="radio" checked={d.packageAfter===p} onChange={() => { upd("packageAfter",p); clr("packageAfter"); }}
                style={{ width:13, height:13, cursor:"pointer", accentColor:C.teal }} />
              {p}
            </label>
          ))}
        </div>
        {te.packageAfter && <div style={{ fontSize:12, color:C.danger, marginTop:4 }}>⚠ {te.packageAfter}</div>}
      </ChkRow>

      <ChkRow checked={d.addConcierge} label="コンシェルジュ追加"
        onChange={e => { upd("addConcierge", e.target.checked); if (!e.target.checked) upd("addConciergeCount",""); clr("_noChange"); }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ fontSize:13, color:C.muted }}>追加台数</span>
          <UISelect value={d.addConciergeCount||""} hasError={!!te.addConciergeCount}
            onChange={e => { upd("addConciergeCount", e.target.value); clr("addConciergeCount"); }}
            style={{ maxWidth:120 }}>
            <option value="">選択</option>
            {CONCIERGE_ADD_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
          </UISelect>
        </div>
        {te.addConciergeCount && <div style={{ fontSize:12, color:C.danger, marginTop:4 }}>⚠ {te.addConciergeCount}</div>}
      </ChkRow>

      <ChkRow checked={d.addLan} label="LANオプション追加"
        onChange={e => { upd("addLan", e.target.checked); if (!e.target.checked) { upd("addLanSetsuchiSha",""); } clr("_noChange"); }}>
        <SetsuchiShaField value={d.addLanSetsuchiSha} onChange={v => { upd("addLanSetsuchiSha",v); clr("addLanSetsuchiSha"); }} error={te.addLanSetsuchiSha} />
      </ChkRow>

      <ChkRow checked={d.addCti} label="CTI追加"
        onChange={e => { upd("addCti", e.target.checked); if (!e.target.checked) upd("addCtiSetsuchiSha",""); clr("_noChange"); }}>
        <SetsuchiShaField value={d.addCtiSetsuchiSha} onChange={v => { upd("addCtiSetsuchiSha",v); clr("addCtiSetsuchiSha"); }} error={te.addCtiSetsuchiSha} />
      </ChkRow>

      {[["addShozai","粧材管理追加"],["addMfCloud","MFクラウド連携追加"],["addBarcodePay","バーコード決済追加"],["addBarcode","バーコード追加"],["addMilbon","ミルボン連携追加"]].map(([key, label]) => (
        <ChkRow key={key} checked={d[key]} label={label}
          onChange={e => { upd(key, e.target.checked); clr("_noChange"); }} />
      ))}
    </Card>

    {/* 解約する内容 */}
    <Card>
      <SecTitle icon="➖">解約する内容</SecTitle>

      <ChkRow checked={d.cancelConcierge} label="コンシェルジュ解約"
        onChange={e => { upd("cancelConcierge", e.target.checked); if (!e.target.checked) upd("cancelConciergeRejiNo",""); clr("_noChange"); }}>
        <Field label="解約するレジNO" required error={te.cancelConciergeRejiNo}>
          <UIInput value={d.cancelConciergeRejiNo||""} hasError={!!te.cancelConciergeRejiNo}
            onChange={e => { upd("cancelConciergeRejiNo", e.target.value); clr("cancelConciergeRejiNo"); }}
            placeholder="例：1" style={{ maxWidth:160 }} />
        </Field>
      </ChkRow>

      <ChkRow checked={d.cancelLan} label="LANオプション解約"
        onChange={e => { upd("cancelLan", e.target.checked); if (!e.target.checked) upd("cancelLanRejiNo",""); clr("_noChange"); }}>
        <Field label="解約するレジNO" required error={te.cancelLanRejiNo}>
          <UIInput value={d.cancelLanRejiNo||""} hasError={!!te.cancelLanRejiNo}
            onChange={e => { upd("cancelLanRejiNo", e.target.value); clr("cancelLanRejiNo"); }}
            placeholder="例：1" style={{ maxWidth:160 }} />
        </Field>
      </ChkRow>

      {[["cancelCti","CTI解約"],["cancelShozai","粧材管理解約"],["cancelMfCloud","MFクラウド連携解約"],["cancelBarcodePay","バーコード決済解約"],["cancelBarcode","バーコード解約"],["cancelMilbon","ミルボン連携解約"]].map(([key, label]) => (
        <ChkRow key={key} checked={d[key]} label={label}
          onChange={e => { upd(key, e.target.checked); clr("_noChange"); }} />
      ))}

      <ChkRow checked={d.cancelRewrite} label="リライト解約"
        onChange={e => { upd("cancelRewrite", e.target.checked); if (!e.target.checked) upd("cancelRewriteRejiNo",""); clr("_noChange"); }}>
        <Field label="解約するレジNO" required error={te.cancelRewriteRejiNo}>
          <UIInput value={d.cancelRewriteRejiNo||""} hasError={!!te.cancelRewriteRejiNo}
            onChange={e => { upd("cancelRewriteRejiNo", e.target.value); clr("cancelRewriteRejiNo"); }}
            placeholder="例：1" style={{ maxWidth:160 }} />
        </Field>
      </ChkRow>

      <ChkRow checked={d.cancelJiki} label="磁気解約"
        onChange={e => { upd("cancelJiki", e.target.checked); if (!e.target.checked) upd("cancelJikiRejiNo",""); clr("_noChange"); }}>
        <Field label="解約するレジNO" required error={te.cancelJikiRejiNo}>
          <UIInput value={d.cancelJikiRejiNo||""} hasError={!!te.cancelJikiRejiNo}
            onChange={e => { upd("cancelJikiRejiNo", e.target.value); clr("cancelJikiRejiNo"); }}
            placeholder="例：1" style={{ maxWidth:160 }} />
        </Field>
      </ChkRow>
    </Card>
    ...
    {needsVisit && (
      <Card>
        ...
        <Field label="日程調整先" required error={te.nicchoseiSaki}>
          <UISelect value={d.nicchoseiSaki||""} hasError={!!te.nicchoseiSaki}
            onChange={e => { upd("nicchoseiSaki", e.target.value); clr("nicchoseiSaki"); }} style={{ maxWidth:240 }}>
            <option value="">選択してください</option>
            <option value="店舗">店舗</option>
            <option value="担当者">担当者</option>
          </UISelect>
        </Field>
        <Field label="担当者名" required error={te.nicchoseiTantosha}>
          <UIInput value={d.nicchoseiTantosha||""} hasError={!!te.nicchoseiTantosha}
            onChange={e => { upd("nicchoseiTantosha", e.target.value); clr("nicchoseiTantosha"); }} placeholder="例：山田 太郎" />
        </Field>
        <Field label="フリガナ" required error={te.nicchoseiTantoshaKana}>
          <UIInput value={d.nicchoseiTantoshaKana||""} hasError={!!te.nicchoseiTantoshaKana} onlyKatakana
            onChange={e => { upd("nicchoseiTantoshaKana", e.target.value); clr("nicchoseiTantoshaKana"); }} placeholder="例：ヤマダ タロウ" />
        </Field>
        <Field label="連絡先電話番号" required hint={!te.nicchoseiDenwa ? "ハイフンあり・なしどちらでも可" : undefined} error={te.nicchoseiDenwa}>
          <UIInput value={d.nicchoseiDenwa||""} hasError={!!te.nicchoseiDenwa} numHyphen
            onChange={e => { upd("nicchoseiDenwa", e.target.value); clr("nicchoseiDenwa"); }} placeholder="例：090-1234-5678" style={{ maxWidth:240 }} />
      ...
      <Card>
        <Field label="変更希望日" required error={te.henkoKiboDate}>
          <UIInput type="date" value={d.henkoKiboDate||""} hasError={!!te.henkoKiboDate}
            onChange={e => { upd("henkoKiboDate", e.target.value); clr("henkoKiboDate"); }}
            style={{ maxWidth:200 }} />
        </Field>
      </Card>
    )}
    ...
    <Card>
      <Field label="備考">
        <UITextarea value={d.biko||""} onChange={e => upd("biko", e.target.value)}
          placeholder="変更内容に関する補足事項があれば記入してください" />
      </Field>
    </Card>
  </>);
};
        ...
        : (() => {
            ...
            return (<>
              ...
              {(activeTab.type === "新規_単店" || activeTab.type === "新規_店舗追加")
                ...
                : (activeTab.type === "端末入替" || activeTab.type === "端末入替_変更あり")
