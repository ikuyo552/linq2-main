...
const UIInput = forwardRef(({ style, hasError, onlyNum, numHyphen, noZenkaku, onlyKatakana, disabled, onChange, ...props }, ref) => {
  const handleChange = e => {
    ...
    if (onlyKatakana)  v = v.replace(/[^\u30A0-\u30FF\uFF65-\uFF9F\u3000 　ー（）]/g, "");
    e.target.value = v;
    onChange && onChange(e);
  };
  return (
...
const FrontWanNewTabContent = ({ tab, form, updTab, clrTab, errors }) => {
  ...
  return (<>
    ...
    <Card>
      ...
      <div style={{ marginTop:14, padding:"12px 16px", background:C.tealLight, borderRadius:10, border:`1px solid ${C.teal}` }}>
    ...
    <Card>
      ...
      <div style={{ padding:"14px 18px", background:C.tealLight, border:`1.5px solid ${C.teal}`, borderRadius:10, fontSize:13.5, color:C.tealDeep, lineHeight:1.8 }}>
        {linkedName
          ? <>📅 日程調整は <strong>「{linkedName}」</strong> と同じになります。<br /><span style={{ fontSize:12.5, color:C.tealDark }}>店舗の申込書タブで日程を入力してください。</span></>
            ...
            <div style={{ marginTop:10 }}>
              {d.conciergeCount === "6〜10台" && (
                <Field label="実際の使用台数" required error={te.conciergeActualCount}>
                  <UISelect value={d.conciergeActualCount||""} style={{ maxWidth:160, borderColor:te.conciergeActualCount?C.danger:undefined }}
