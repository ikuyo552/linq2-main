  ...
  return (
    <div style={ps.page}>
      ...
      <table style={ps.table}><tbody>
        ...
        {OPTIONS_LIST.filter(o => o.key !== "cti").map(opt => {
          const k = KH_OPT_KEY[opt.key];
          return d[k] ? <TR key={opt.key} label={opt.label} value={opt.price > 0 ? `¥${opt.price.toLocaleString()}` : "¥0"} /> : null;
        })}
        ...
        {d.rewrite && <TR label="リライト" value={d.rewrite} />}
        {d.jiki    && <TR label="磁気"     value={d.jiki} />}
        {khTotal !== null && (
          <tr>
            <td style={{ ...ps.tdLabel, background:"#1a3358", color:"#fff", fontWeight:700 }}>月額合計（税別）</td>
